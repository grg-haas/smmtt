...
connect-$(1):
	PYTHONPATH=$$(LINUX$(1)_BUILDDIR)/scripts/gdb \
		$$(CROSS_COMPILE$(1))gdb -x $$(SMMTT)/scripts/gdb/gdb$(2).cfg
...
ifeq ($(1),32)
	QEMU$(3)-$(2)$(1)$(4)_RUN_FLAGS += -m 1536M
else ifeq ($(1),64)
...
$$(eval $$(call __run-target,$(3)-$(2)$(1)))
$$(eval $$(call __dbg-target,$(3)-$(2)$(1)))
...
$(3)_RUN_FLAGS += \
	-device loader,file=$$($(3)_RUN_FILE),addr=0xBC000000,force-raw=on \
	-device opensbi-memregion,id=mem,base=0xBC000000,size=0x4000000,mmio=false \

# Add test memory regions
$(3)_RUN_FLAGS += \
	-device opensbi-memregion,id=oneg,base=0xC0000000,size=0x40000000,mmio=false \
	-device opensbi-memregion,id=twom,base=0x100000000,size=0x200000,mmio=false \
	-device opensbi-memregion,id=l1,base=0x100200000,size=0x1000,mmio=false

$(3)_BOOT = possible-harts=0-1,boot-hart=0x0,next-addr=0xBC000000,next-arg1=0xBFE00000,next-mode=1,smmtt-mode=$(4)
$(3)_REGIONS = region0=mem,perms0=$(5),region1=oneg,perms1=$(5),region2=twom,perms2=$(5),region3=l1,perms3=$(5)
...
$$(eval $$(call __run-target,unittests-smmtt$(1)-$(2)))
$$(eval $$(call __dbg-target,unittests-smmtt$(1)-$(2)))
$$(eval $$(call __connect-target,unittests-smmtt$(1)-$(2)))
...
SMMTT1_PERM_VALUES	:= 0x7 0xf 0x1f
SMMTT0_PERM_VALUES	:= 0x7 0x3f
